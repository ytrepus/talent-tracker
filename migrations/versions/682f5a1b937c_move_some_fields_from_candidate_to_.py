...
def upgrade():
    ...
    op.add_column('application', sa.Column('scheme_start_date', sa.Date(), nullable=True))
    op.create_index(op.f('ix_application_scheme_start_date'), 'application', ['scheme_start_date'], unique=False)
    op.drop_index('ix_candidate_scheme_start_date', table_name='candidate')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('candidate', sa.Column('scheme_start_date', sa.DATE(), autoincrement=False, nullable=True))
    op.add_column('candidate', sa.Column('scheme', sa.VARCHAR(length=10), autoincrement=False, nullable=True))
    op.create_index('ix_candidate_scheme_start_date', 'candidate', ['scheme_start_date'], unique=False)
