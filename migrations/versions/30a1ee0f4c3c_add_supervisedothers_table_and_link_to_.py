...
def upgrade():
    ...
    op.add_column(
        "socio_economic", sa.Column("supervised_others_id", sa.Integer(), nullable=True)
    )
    op.create_foreign_key(
        None, "socio_economic", "supervised_others", ["supervised_others_id"], ["id"]
    )
...
def downgrade():
    ...
    op.add_column(
        "socio_economic",
        sa.Column("supervisor", sa.BOOLEAN(), autoincrement=False, nullable=True),
    )
    op.drop_constraint(
        "socio_economic_supervised_others_id_fkey", "socio_economic", type_="foreignkey"
    )
